...
class ConnectionBoundUndoTest {
    ...
    @Test
    void undoLogRollsBackWithTheBusinessConnection() throws Exception {
        JdbcDataSource raw = dataSource("connectionBoundRollback");
        createSchema(raw);
        ...
        AtTransactionManager manager =
                new AtTransactionManager(
                        ...
                        new JdbcGlobalLockManager(raw, 10000),
                        3);
        AtDataSource dataSource =
                new AtDataSource("dataSource", raw, manager, new JdbcGlobalLockManager(raw, 10000));
        ...
        assertTrue(
                repository.find(tx.getXid()).get().getUndoRecords().isEmpty(),
                "undo log must roll back with business DML");
        manager.rollback(tx.getXid());
    }

    @Test
    void committedLocalTransactionPersistsUndoForGlobalRollback() throws Exception {
        JdbcDataSource raw = dataSource("connectionBoundCommit");
        createSchema(raw);
        JdbcAtRepository repository = new JdbcAtRepository(raw);
    ...
    private void createSchema(DataSource ds) throws SQLException {
    ...
    private int balance(DataSource ds) throws SQLException {
        try (Connection c = ds.getConnection();
                Statement s = c.createStatement();
