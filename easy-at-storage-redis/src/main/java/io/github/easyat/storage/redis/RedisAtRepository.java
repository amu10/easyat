 ...
 * so multiple recovery instances cannot advance the same transaction concurrently. Recovery ownership
 * and the recovery scan use per-status sets.
 */
public final class RedisAtRepository implements AtRepository {
    private final JedisPool pool; private final UndoDataCodec codec; private final String prefix;
    public RedisAtRepository(JedisPool pool){this(pool,new JacksonUndoDataCodec(),"easy-at");}
    public RedisAtRepository(JedisPool pool,UndoDataCodec codec,String prefix){this.pool=pool;this.codec=codec;this.prefix=prefix==null?"easy-at":prefix;}

    private String gkey(String xid){return prefix+":global:"+xid;}
    private String statusSet(String status){return prefix+":global:status:"+status;}
    private String undoKey(String xid){return prefix+":undo:"+xid;}
    private String undoIdKey(String xid,String undoId){return prefix+":undo:"+xid+":"+undoId;}
    ...
    @Override public void create(AtTransaction tx){
        try(Jedis j=pool.getResource()){
            ...
            m.put("name",tx.getName()); m.put("status",tx.getStatus().name());
            m.put("timeout_at",String.valueOf(tx.getDeadline()));
            m.put("retry_count","0"); m.put("next_retry_at","0");
            m.put("version","0"); m.put("owner",""); m.put("lease_until","0");
            long now=System.currentTimeMillis(); m.put("created_at",String.valueOf(now)); m.put("updated_at",String.valueOf(now));
    ...
    @Override public Optional<AtTransaction> find(String xid){
        try(Jedis j=pool.getResource()){
            ...
            AtTransaction tx=new AtTransaction(xid,m.get("name"),created,Long.parseLong(m.getOrDefault("timeout_at","0")));
            long next=Long.parseLong(m.getOrDefault("next_retry_at","0"));
            tx.restore(AtStatus.valueOf(m.get("status")),Integer.parseInt(m.getOrDefault("retry_count","0")),next);
            tx.setVersion(Long.parseLong(m.getOrDefault("version","0")));
            String owner=m.get("owner"); if(owner!=null&&!owner.isEmpty())tx.setOwner(owner);
            long lease=Long.parseLong(m.getOrDefault("lease_until","0")); if(lease>0)tx.setLeaseUntil(lease);
            loadUndo(j,tx);
    ...
    private void loadUndo(Jedis j,AtTransaction tx){
        for(String id:j.smembers(undoKey(tx.getXid()))){
            ...
            Object[] params=m.containsKey("rollback_params")?codec.decodeParameters(b64(m.get("rollback_params")),ctx):new Object[0];
            UndoRecord u=new UndoRecord(id,tx.getXid(),m.get("resource_id"),m.get("table_name"),m.get("pk_name"),m.get("pk_value"),m.get("rollback_sql"),params);
            if(m.containsKey("before_image"))u.setBeforeImage(codec.decodeRowImage(b64(m.get("before_image")),ctx));
            if(m.containsKey("after_image"))u.setAfterImage(codec.decodeRowImage(b64(m.get("after_image")),ctx));
    ...
    @Override public void save(AtTransaction tx){
        try(Jedis j=pool.getResource()){
            ...
            for(UndoRecord u:tx.getUndoRecords()){
                ...
                m.put("resource_id",u.getResourceId()); m.put("table_name",u.getTableName());
                m.put("pk_name",u.getPrimaryKeyColumn()); m.put("pk_value",String.valueOf(u.getPrimaryKeyValue()));
                m.put("rollback_sql",u.getRollbackSql());
                m.put("rollback_params",b64(codec.encodeParameters(u.getParameters(),ctx)));
                if(u.getBeforeImage()!=null)m.put("before_image",b64(codec.encodeRowImage(u.getBeforeImage(),ctx)));
                if(u.getAfterImage()!=null)m.put("after_image",b64(codec.encodeRowImage(u.getAfterImage(),ctx)));
    ...
    @Override public boolean transition(String xid,AtStatus expected,long expectedVersion,AtStatus next){
        try(Jedis j=pool.getResource()){
            Object res=j.eval(TRANSITION_LUA,3,gkey(xid),statusSet(expected.name()),statusSet(next.name()),
                xid,expected.name(),String.valueOf(expectedVersion),next.name(),String.valueOf(System.currentTimeMillis()));
            return "1".equals(String.valueOf(res));
        }
    }
    @Override public boolean claimLease(String xid,String owner,long leaseUntil,long now){
        try(Jedis j=pool.getResource()){
            Object res=j.eval(CLAIM_LUA,1,gkey(xid),owner,String.valueOf(leaseUntil),String.valueOf(now));
            return "1".equals(String.valueOf(res));
        }
    }
    @Override public void releaseLease(String xid,String owner){
        try(Jedis j=pool.getResource()){j.eval(RELEASE_LUA,1,gkey(xid),owner);}catch(Exception ignored){}
    }
    ...
    @Override public List<AtTransaction> recoverable(long now,int limit){
        ...
        for(String xid:candidates){
            ...
            else if(t.getStatus()==AtStatus.ROLLBACK_FAILED&&t.getNextRetryAt()<=now)out.add(t);
