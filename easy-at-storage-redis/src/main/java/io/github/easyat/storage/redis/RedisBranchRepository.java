...
public final class RedisBranchRepository implements BranchRepository {
    ...
    public void register(AtBranch b) {
        try (Jedis j = pool.getResource()) {
            write(j, b);
            j.sadd(byXidKey(b.getXid()), b.getBranchId());
            j.sadd(statusSet(b.getStatus().name()), b.getBranchId());
        }
    }

    private void write(Jedis j, AtBranch b) {
        Map<String, String> m = new HashMap<String, String>();
        m.put("branch_id", b.getBranchId());
        m.put("xid", b.getXid());
        m.put("resource_id", b.getResourceId());
        m.put("status", b.getStatus().name());
        m.put("service_name", b.getServiceName() == null ? "" : b.getServiceName());
        m.put("callback_url", b.getCallbackUrl() == null ? "" : b.getCallbackUrl());
        m.put("sequence", String.valueOf(b.getSequence()));
        m.put("retry_count", String.valueOf(b.getRetries()));
        m.put("created_at", String.valueOf(b.getCreatedAt()));
        m.put("updated_at", String.valueOf(b.getUpdatedAt()));
