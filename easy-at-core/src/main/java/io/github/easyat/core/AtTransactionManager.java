 ...
 *
 * <p>这是框架的「指挥中枢」，但本身不保存任何业务状态——所有决定恢复结果的状态
 * （状态机、undo 记录、租约、重试计数）都持久化到 {@link AtRepository}。
 * 内存里的 {@link AtTransaction} 只是存储快照，真正的并发正确性由存储层的 CAS 保证。
 *
 * <p>关键设计（对应 DESIGN.md §4）：
 * <ul>
 *   <li><b>状态迁移必须 CAS</b>：{@link #transition} 把「期望状态 + 版本号」作为 WHERE 条件
 *       提交给存储层，多实例并发回滚同一 XID 时只有一个实例能改成功，其余拿不到
 *       （返回 false）从而放弃，避免重复补偿。</li>
 *   <li><b>回滚按逆序执行 undo</b>：先写后回滚，符合 AT 的补偿顺序。</li>
 *   <li><b>失败进入退避重试</b>：{@link #backoff} 指数退避，上限 5 分钟，配合
 *       {@link #recover} 与 {@link AtRecoveryScheduler} 实现自动恢复。</li>
 * </ul>
 */
public final class AtTransactionManager {
    ...
    public AtTransactionManager(AtRepository r, UndoExecutor u, GlobalLockManager locks, int maxRetries) {
        repository = r;
        undoExecutor = u;
        lockManager = locks;
    ...
    public AtTransaction begin(String name, long timeout) {
        ...
        AtTransaction tx = new AtTransaction(UUID.randomUUID().toString(), name, now, now + timeout);
    ...
    /**
     * 回滚：进入 ROLLING_BACK 后，把 undo 记录<b>逆序</b>逐条执行。
     * 任一条 undo 抛出 {@link DirtyWriteException} 则整笔转 DIRTY_WRITE（脏写，人工介入）；
     * 其他异常转 ROLLBACK_FAILED 并按退避时间写入下一次重试点。
     */
    ...
    /**
     * 恢复一个待处理事务：重试次数耗尽则转 MANUAL_INTERVENTION（人工介入），
     * 否则自增重试计数、写退避后的下一次重试时间，再执行回滚。
     */
    ...
    /**
     * 状态迁移核心：先做内存合法性校验，再以 CAS 提交到存储层。
     * CAS 成功才更新内存快照的版本号，失败返回 false（说明被其他实例抢先）。
     */
    ...
    private AtTransaction required(String xid) {
        return repository.find(xid).orElseThrow(() -> new AtException("Transaction not found: " + xid));
    }
