...
public final class HmacSigner {
    ...
    public HmacSigner(String secret) {
        this.secret =
                secret == null ? null : secret.getBytes(java.nio.charset.StandardCharsets.UTF_8);
    }
     ...
     * 做恒定时间比较，防时序侧信道。注意：本方法<b>不校验 source 白名单</b>，仅验证「持有共享 密钥方发来的合法签名」，服务间认证需在上层叠加 source
     * 白名单（见生产安全缺口清单）。
     */
    ...
    private byte[] hmac(String data) {
        try {
            ...
            throw new AtException("Cannot compute HMAC", e);
        }
    }

    /** 签名原文固定为三行拼接，顺序与 header 保持一致，任何字段变动都会导致签名失效。 */
    private static String payload(String xid, long deadline, String source) {
        return xid + "\n" + deadline + "\n" + source;
    }
