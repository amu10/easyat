 ...
 * skips the bean.
 *
 * <p><b>为什么用反射：</b>离线构建环境没有 {@code spring-webflux} 依赖，若直接 import {@code ExchangeFilterFunction}
 * 等类型会让编译失败。这里通过 {@code Class.forName} + JDK 动态代理，在运行时才解析 WebClient 的类与方法，因此对 webflux 的依赖是「可选的」：
        ...
        try {
            ...
            Object filter =
                    Proxy.newProxyInstance(
                            WebClientPropagator.class.getClassLoader(),
                            new Class<?>[] {filterIface},
                            new ExchangeFilterHandler(
                                    signer,
                                    appName,
                                    mutate,
                                    header,
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            if (xid != null) {
                ...
                if (signer != null && signer.isConfigured())
                    header.invoke(
                            builder,
                            AtTransportHeaders.SIGNATURE,
                            signer.sign(xid, deadline, appName));
