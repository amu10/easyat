...
/**
 * Verifies cross-service headers (HMAC signature, deadline, source).
 * Works on a plain header map so it is usable from both Boot 2 (javax) and Boot 3 (jakarta) controllers.
 * Allows traffic when no secret is configured (dev mode); in {@code production=true} a secret is mandatory and
 * unauthenticated traffic is rejected by the caller.
 */
public final class EasyAtTransportSecurity {
    private final HmacSigner signer; private final boolean production;
    public EasyAtTransportSecurity(HmacSigner signer,boolean production){this.signer=signer;this.production=production;}
    public boolean authorized(Map<String,String> headers){
        ...
        try{return signer.verify(xid,Long.parseLong(deadline),source,signature);}catch(NumberFormatException e){return false;}
    }
