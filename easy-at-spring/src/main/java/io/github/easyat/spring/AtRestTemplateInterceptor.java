...
public final class AtRestTemplateInterceptor implements ClientHttpRequestInterceptor {
    ...
    public AtRestTemplateInterceptor(){this(null,"");}
    public AtRestTemplateInterceptor(HmacSigner signer,String appName){this.signer=signer;this.appName=appName;}
    @Override public ClientHttpResponse intercept(HttpRequest request,byte[] body,ClientHttpRequestExecution execution)throws IOException{
        ...
        if(xid!=null){
            ...
            if(signer!=null&&signer.isConfigured())request.getHeaders().set(AtTransportHeaders.SIGNATURE,signer.sign(xid,deadline,appName));
        }
