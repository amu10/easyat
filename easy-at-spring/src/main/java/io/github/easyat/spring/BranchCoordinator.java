...
public final class BranchCoordinator {
    ...
    public synchronized String register(
        ...
        for (AtBranch b : branches.byXid(xid))
            if (b.getResourceId().equals(resourceId)) return b.getBranchId();
        AtBranch b =
                new AtBranch(
                        xid,
                        resourceId,
    ...
    private void drive(String xid, boolean rollback) {
        List<AtBranch> list = branches.byXid(xid);
        boolean localHandled = false;
        for (AtBranch b : list) {
            if (b.getStatus().isTerminal()) continue;
            if (isLocal(b)) {
                if (!localHandled) {
                    if (rollback) manager.rollback(xid);
                    localHandled = true;
                }
    ...
    private BranchStatus deliverRemote(AtBranch b, boolean rollback) {
        try {
            ...
            ResponseEntity<String> res =
                    rest.postForEntity(url, new HttpEntity<String>(headers), String.class);
